...
uint8_t battery_life_percent(const struct battery_list *batt_list)
{
    ...
    if (!batt_list || !batt_list->count) return (uint8_t)-1;
    ...
    return (uint8_t)(total_cap * 1.0 / batt_list->count);
}
...
uint32_t battery_life_time(const struct battery_list *batt_list)
{
    ...
    if (ac_connected() || !batt_list || !batt_list->count) return (uint32_t)-1;
    ...
    return (uint32_t)remaining_time;
}
